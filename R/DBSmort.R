...
#' @param removed              POSIXct object. The first date not considered part of the defunct cluster. The date should be moved forward if collars were moved from the study site and a secondary cluster was created. A date time must be listed in "YYYY-MM-DDTHH:MM:SS" format. Dates are downloaded in UTC+0.
...
#' @examples 
#' DBSmort(id = 39966,
#'          date = '2024-01-28T15:00:38',
#'          removed = '2024-02-15T21:01:39',
#'          zeta = 30,
#'          eta = 2,
#'          theta = 6,
#'          reason = "cougar",
...
DBSmort <- function(id, date, removed, zeta, eta, theta, reason, filepath, keypath){ 
  ...
  lastalive = as.data.frame(as.data.frame(check) %>% filter(defunct_cluster_ %in% FALSE) %>% group_by(defunct_cluster_) %>% filter(t_ != max(t_)) %>% arrange(desc(t_)) %>% slice(1))
  firstclust = as.data.frame(as.data.frame(check) %>% filter(defunct_cluster_ %in% FALSE) %>% group_by(defunct_cluster_) %>% arrange(desc(t_)) %>% slice(1))
  lastclust = as.data.frame(as.data.frame(check) %>% filter(defunct_cluster_ %in% TRUE) %>% group_by(defunct_cluster_) %>% arrange(desc(t_)) %>% slice(1))
  ...
  message = as.character(message)
  if(message == "[accurate]"){
    metadata = metadata %>% mutate(lastlivefix = as.character(lastlivefix)) %>% mutate(lastlivefix = ifelse(collarid == id, as.character(lastalive[1,3]),lastlivefix)) %>%
      mutate(firstfixinendclust = as.character(firstfixinendclust )) %>% mutate(firstfixinendclust  = ifelse(collarid == id, as.character(firstclust[1,3]),firstfixinendclust)) %>%
      mutate(lastfixinendclust = as.character(lastfixinendclust)) %>% mutate(lastfixinendclust  = ifelse(collarid == id, as.character(lastclust[1,3]),lastfixinendclust)) %>%
      ...
      mutate(lastlivefix = as.POSIXct(lastlivefix, format = '%Y-%m-%d %H:%M:%S', tz = 'UTC')) %>%
      mutate(firstfixinendclust = as.POSIXct(firstfixinendclust, format = '%Y-%m-%d %H:%M:%S', tz = 'UTC')) %>%
      mutate(lastfixinendclust = as.POSIXct(lastfixinendclust, format = '%Y-%m-%d %H:%M:%S', tz = 'UTC'))
    ...
    if(message == "[yes]"){
      ...
      dir.create(path = paste0(gsub("metadata_updated.xlsx", "DBSmort outputs",filepath)))
      openxlsx::write.xlsx(metadata,paste0(gsub("metadata_updated.xlsx", "DBSmort outputs",filepath), '\\metadata.xlsx'), keepNA = FALSE)
    }else{
